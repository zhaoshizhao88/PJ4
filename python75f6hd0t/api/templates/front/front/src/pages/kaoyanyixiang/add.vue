<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="学历" prop="xueli">
			...
			<el-form-item class="add-item"  label="意向层次" prop="yixiangcengci">
				<el-select v-model="ruleForm.yixiangcengci" placeholder="请选择意向层次" :disabled=" false  ||ro.yixiangcengci" >
					<el-option
						v-for="(item,index) in yixiangcengciOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
			...
			<el-form-item class="add-item" label="成绩分数" prop="chengjifenshu">
				<el-input-number v-model="ruleForm.chengjifenshu" placeholder="成绩分数" :disabled=" false ||ro.chengjifenshu"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="所在地" prop="suozaidi">
				<el-input v-model="ruleForm.suozaidi" 
					placeholder="所在地" clearable :disabled=" false  ||ro.suozaidi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="可选地" prop="kexuandi">
				<el-input v-model="ruleForm.kexuandi" 
					placeholder="可选地" clearable :disabled=" false  ||ro.kexuandi"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="意向偏好" prop="yixiangpianhao">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="意向偏好"
					v-model="ruleForm.yixiangpianhao">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="意向详情" prop="yixiangxiangqing">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yixiangcengci'){
							this.ruleForm.yixiangcengci = obj[o];
							this.ro.yixiangcengci = true;
							continue;
						}
						if(o=='chengjifenshu'){
							this.ruleForm.chengjifenshu = obj[o];
							this.ro.chengjifenshu = true;
							continue;
						}
						if(o=='suozaidi'){
						...
						if(o=='kexuandi'){
							this.ruleForm.kexuandi = obj[o];
							this.ro.kexuandi = true;
							continue;
						}
						if(o=='dengjishijian'){
						...
						if(o=='yixiangpianhao'){
							this.ruleForm.yixiangpianhao = obj[o];
							this.ro.yixiangpianhao = true;
							continue;
						}
				...
				this.yixiangcengciOptions = "985,211,普通院校".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`kaoyanyixiang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`kaoyanyixiang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
