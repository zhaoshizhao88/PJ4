<template>
	<div class="main-content" :style='{"width":"calc(100% - 35px)","margin":"10px 20px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 10px","margin":"0 10px 20px 10px","borderRadius":"20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","alignItems":"center","flexWrap":"wrap","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">专业</label>
				...
				<el-row class="actions" :style='{"padding":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('kaoyanyixiang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kaoyanyixiang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"16px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaoyanyixiang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('kaoyanyixiang','推荐院校')" @click="tuijianyuanxiaoCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								推荐院校
							</el-button>
							<el-button class="edit" v-if=" isAuth('kaoyanyixiang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('kaoyanyixiang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<tuijianyuanxiao-cross-add-or-update v-if="tuijianyuanxiaoCrossAddOrUpdateFlag" :parent="this" ref="tuijianyuanxiaoCrossaddOrUpdate"></tuijianyuanxiao-cross-add-or-update>
...
<script>
	...
	import tuijianyuanxiaoCrossAddOrUpdate from "../tuijianyuanxiao/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				tuijianyuanxiaoCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			tuijianyuanxiaoCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			tuijianyuanxiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tuijianyuanxiaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kaoyanyixiang');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tuijianyuanxiaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tuijianyuanxiaoCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				if(this.searchForm.yixiangcengci!='' && this.searchForm.yixiangcengci!=undefined){
					params['yixiangcengci'] = this.searchForm.yixiangcengci
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "kaoyanyixiang/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
